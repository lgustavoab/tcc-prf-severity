...
EXPECTED_XGBOOST_VERSION = "3.3.0"
MODEL_ID = "phase_4c_xgboost_baseline"
CALIBRATION_BINS = 10


@dataclass(frozen=True)
class FoldXGBoostResult:
    fold: int
...
class XGBoostBaselineResult:
    fold_results: tuple[FoldXGBoostResult, ...]
...
@dataclass(frozen=True)
class XGBoostBaselineRun:
    result: XGBoostBaselineResult
...
def validate_xgboost_version(version: str | None = None) -> str:
    """Garante que a família seja executada na versão congelada pelo contrato 4C."""
    installed = xgboost.__version__ if version is None else version
    if installed != EXPECTED_XGBOOST_VERSION:
        raise RuntimeError(
            f"XGBoost incompatível: esperado {EXPECTED_XGBOOST_VERSION}, encontrado {installed}."
        )
    return installed


def build_xgboost_pipeline(groups: PreprocessingGroups) -> Pipeline:
    """Cria uma instância não fitada com a configuração XGBoost congelada."""
...
def extract_xgboost_positive_probability(
    classifier: XGBClassifier,
    predicted_probabilities: np.ndarray,
) -> np.ndarray:
    """Extrai explicitamente P(classe 1), preservando True como grave."""
    classes = np.asarray(classifier.classes_)
    numeric_classes = all(
        isinstance(value, (int, np.integer)) and not isinstance(value, (bool, np.bool_))
        for value in classes
    )
    if classes.shape != (2,) or not numeric_classes or set(classes.tolist()) != {0, 1}:
        raise ValueError(
            "XGBClassifier.classes_ deve conter exatamente as classes numéricas 0 e 1."
        )
    positive_indices = np.flatnonzero(classes == 1)
    if len(positive_indices) != 1:
        raise ValueError("A classe numérica 1/grave não foi localizada de forma unívoca.")

    raw = np.asarray(predicted_probabilities)
    if raw.ndim != 2 or raw.shape[1] != len(classes):
        raise ValueError("predict_proba deve produzir uma matriz com uma coluna por classe.")
    probabilities = np.asarray(raw[:, int(positive_indices[0])], dtype=float)
...
def _validate_modeling_fold(fold: TemporalFold) -> None:
    if FINAL_TEST_YEAR in (*fold.train_years, fold.validation_year):
        raise ValueError("2025 é proibido em treino, transformação e avaliação da Fase 4C.")
...
def _audit_boosting_rounds(classifier: XGBClassifier) -> tuple[int, int, int]:
    configured = classifier.n_estimators
    max_depth = classifier.max_depth
    if not isinstance(configured, int) or not isinstance(max_depth, int):
        raise TypeError("n_estimators e max_depth devem ser inteiros explícitos no contrato 4C.")
    completed = int(classifier.get_booster().num_boosted_rounds())
    if completed != configured:
        raise ValueError(
            f"XGBoost completou {completed} rounds, mas a configuração exige {configured}."
        )
    return configured, completed, max_depth
...
def run_xgboost_fold(
    ...
    fold: TemporalFold,
    pipeline_factory: PipelineFactory = build_xgboost_pipeline,
) -> FoldXGBoostResult:
    """Ajusta um XGBoost novo somente no treino e avalia uma validação temporal."""
    ...
    probabilities = extract_xgboost_positive_probability(
        classifier,
    ...
    configured_rounds, completed_rounds, max_depth = _audit_boosting_rounds(classifier)
    ...
    return FoldXGBoostResult(
        ...
        all_rounds_completed=configured_rounds == completed_rounds,
        oof_predictions=oof,
        calibration=_calibration_table(fold, validation_y, probabilities),
    )


def build_fold_metrics_table(results: tuple[FoldXGBoostResult, ...]) -> pl.DataFrame:
    return pl.DataFrame(
        {
            "fold": result.fold,
            ...
            "configured_boosting_rounds": result.configured_boosting_rounds,
            "completed_boosting_rounds": result.completed_boosting_rounds,
            "max_depth_configured": result.max_depth_configured,
            "all_rounds_completed": result.all_rounds_completed,
        }
        for result in results
    )


def build_xgboost_summary(fold_metrics: pl.DataFrame) -> pl.DataFrame:
    ...
    all_rounds = bool(fold_metrics.get_column("all_rounds_completed").all())
    values: tuple[tuple[str, Any], ...] = (
        ...
        ("all_boosting_rounds_completed", all_rounds),
...
def build_xgboost_model_contract() -> pl.DataFrame:
    version = validate_xgboost_version()
    values = (
        ("library", "xgboost"),
...
def analyze_xgboost_baseline(
    df: pl.DataFrame,
    groups: PreprocessingGroups,
    folds: tuple[TemporalFold, ...],
    pipeline_factory: PipelineFactory = build_xgboost_pipeline,
) -> XGBoostBaselineResult:
    """Executa apenas os três folds internos, sem tuning, threshold ou early stopping."""
    if folds != build_temporal_folds():
        raise ValueError("A Fase 4C aceita somente os folds congelados na Fase 3D.")
    validate_temporal_design(df, groups.predictors, folds)
    results = tuple(run_xgboost_fold(df, groups, fold, pipeline_factory) for fold in folds)
    ...
    return XGBoostBaselineResult(
        ...
        summary=build_xgboost_summary(fold_metrics),
        model_contract=build_xgboost_model_contract(),
...
def write_xgboost_artifacts(
    ...
    outputs = (
        (result.fold_metrics, "phase_4c_xgboost_fold_metrics.csv"),
        (result.summary, "phase_4c_xgboost_summary.csv"),
        (result.model_contract, "phase_4c_xgboost_model_contract.csv"),
        (result.calibration, "phase_4c_xgboost_calibration.csv"),
    )
...
def run_xgboost_baseline(
    ...
    """Verifica a cadeia 3C-3E e executa XGBoost somente até 2024."""
    validate_xgboost_version()
    ...
    _validate_authoritative_contracts(
        groups, experimental_contract_path, preprocessing_contract_path
    )
    result = analyze_xgboost_baseline(pl.read_parquet(analytical_path), groups, folds)
    table_paths, published_oof_path = write_xgboost_artifacts(result, tables_dir, oof_path)
    return XGBoostBaselineRun(
        result=result,
